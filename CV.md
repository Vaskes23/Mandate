...
### Innovation 3: Temporal Signature Analysis
**Concept:** Birds have characteristic "flicker" patterns as they flap wings; static objects don't.

**Theory:**
- A flying bird's silhouette changes shape rapidly (wing beats ~10-15 Hz)
- This creates high-frequency intensity variations at the bird's location
- Static objects (even with lighting changes) have lower frequency variations

**Algorithm:**
```python
# Maintain intensity history for each pixel (last 10 frames)
pixel_history[y, x] = rolling_buffer(frame_gray[y, x])

# Compute frequency content
freq_spectrum = fft(pixel_history[y, x])

# High-frequency energy indicates wing motion
flicker_map = sum(freq_spectrum[5:15])  # 5-15 Hz range

# Combine with MOG2 mask
bird_mask = mog2_mask & (flicker_map > flicker_threshold)
```

**Benefits:**
- Distinguishes birds from swaying branches (different frequency)
- Detects hovering birds that don't move spatially
- Works even for birds flying directly toward/away from camera

### Innovation 4: Shadow-Based Detection
**Concept:** On sunny days, birds cast moving shadows on buildings/ground that can be tracked.

**Theory:**
- Bird shadows appear on static surfaces
- Shadow movement correlates with bird movement
- Shadows may be larger than the bird itself (magnified by distance)

**Algorithm:**
```python
# Detect shadows on known static regions (buildings)
shadow_mask = detect_dark_regions_on_static_surfaces(frame, static_mask)

# Track shadow movement with optical flow
shadow_flow = lucas_kanade(prev_shadow_mask, shadow_mask)

# Correlate shadow positions with expected bird trajectories
# Shadow moves opposite to sun direction relative to bird
bird_estimate = shadow_position + sun_angle_offset
```

**Benefits:**
- Provides redundant detection (bird + shadow = 2 chances to detect)
- Shadow may be visible when bird flies in front of building
- Works when bird is too small to detect directly

### Innovation 5: Acoustic Correlation (Multi-Modal)
**Concept:** If video has audio, bird calls can help validate/predict bird presence.

**Theory:**
- Many birds vocalize while flying
- Audio detection is computationally cheap
- Sound can predict visual detection: if we hear birds, search harder for them

**Algorithm:**
```python
# Analyze audio spectrum for bird frequencies (2-8 kHz typical)
audio_spectrum = fft(audio_chunk)
bird_band_energy = sum(audio_spectrum[2000:8000])

# If bird sounds detected, boost detection sensitivity
if bird_band_energy > threshold:
    mog2_var_threshold *= 0.7  # More sensitive
    min_contour_area *= 0.5    # Accept smaller detections
```

**Benefits:**
- Audio processing is very CPU-efficient
- Provides advance warning of bird presence
- Works even when birds are temporarily off-screen

### Innovation 6: Trajectory Clustering for Static Object ID
**Concept:** Objects with similar, non-moving trajectories are likely static or noise.

**Theory:**
- Real birds have diverse flight paths
- Noise/static objects repeatedly trigger in same locations
- Cluster trajectories by similarity; reject repetitive patterns

**Algorithm:**
```python
# Maintain history of all detected trajectory starts
trajectory_starts = [(x, y, timestamp) for all detections]

# Cluster by spatial proximity
clusters = dbscan(trajectory_starts, eps=10, min_samples=5)

# Clusters with high density = probable static objects
for cluster in clusters:
    if temporal_variance(cluster) < threshold:
        # Same location triggers repeatedly = static noise
        add_to_auto_exclusion_zone(cluster.centroid)
```

**Benefits:**
- Self-learning: identifies problematic areas over time
- No manual configuration needed
- Adapts to new static objects (e.g., someone parks a car)

### Innovation 7: Inverse Detection (Background Modeling Enhancement)
**Concept:** Build a "bird probability map" based on where birds have previously flown.

**Theory:**
- Birds tend to fly in certain regions of the frame (sky)
- They avoid certain regions (behind buildings, below horizon)
- Historical flight paths can weight current detections

**Algorithm:**
```python
# Accumulate all bird trajectories into heat map
bird_probability_map = zeros(frame_shape)

for trajectory in all_historical_trajectories:
    for point in trajectory:
        bird_probability_map[point.y, point.x] += 1

# Normalize to probability
bird_probability_map /= bird_probability_map.max()

# Weight new detections by historical probability
for detection in current_detections:
    detection.confidence *= bird_probability_map[detection.cy, detection.cx]
    if detection.confidence < low_threshold:
        reject(detection)  # Unlikely to be a real bird
```

**Benefits:**
- Buildings, lamp posts, ground have near-zero probability
- Sky regions where birds fly have high probability
- Self-reinforcing: accurate detections improve the map
...
## Experimental Ideas (Research Required)

### Idea A: Lightweight Neural Network (CPU-Optimized)
While the constraint is "no GPU," modern CPUs can run small neural networks efficiently:

- **MobileNet-SSD**: Object detection at ~5-10 FPS on CPU
- **YOLO-Nano**: Designed for edge devices, ~10 FPS on modern CPU
- **Custom 3-layer CNN**: Train specifically for "bird vs. sky vs. structure"

**Trade-off:** Would require training data and significantly increases complexity, but could dramatically improve accuracy for this specific use case.

### Idea B: Crowdsourced Exclusion Zone Learning
- Ship app with ability to upload anonymized exclusion zone masks
- Aggregate across users to build "common obstacle patterns"
- Pre-load exclusion zones for common scenarios (lamp posts, building edges)

### Idea C: Temporal Super-Resolution
Instead of upscaling spatially, use multiple frames to build higher-quality detection:

```python
# Align last 5 frames using optical flow
aligned_frames = [warp(frame[t-i], flow_to_current) for i in range(5)]

# Average to reduce noise while preserving moving birds
# (Birds appear at different positions, so they don't get averaged out)
enhanced = median(aligned_frames)

# Subtract enhanced background from current to isolate birds
birds_only = current_frame - enhanced
```
